...
const TimetableApp = () => {
  ...
  const getAnimationClasses = () => {
    const baseClasses = 'data-[state=active]:animate-in data-[state=inactive]:animate-out data-[state=inactive]:fade-out-50 data-[state=active]:fade-in-50 select-none';
    return `${baseClasses} ${
      direction === 'right' 
  ...
  const handleSwipe = (e) => {
    ...
    if (distance > 0 && currentIndex > 0) {
      ...
      const newDay = days[currentIndex - 1];
      ...
      <Tabs 
        ...
        {days.map((day) => (
          <TabsContent 
            key={day} 
            value={day}
            className={`${getAnimationClasses()} touch-pan-y`}
